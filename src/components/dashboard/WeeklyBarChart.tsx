...
function CustomTooltip({
  ...
  return (
    <div className="rounded-xl border border-pink-200 bg-white px-3 py-2 text-sm shadow-lg shadow-pink-100">
      <span className="font-medium text-pink-900">{formatMinutesHm(minutes)}</span>
    </div>
  );
}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={chartData} margin={{ top: 4, right: 4, left: -16, bottom: 0 }}>
            ...
            <YAxis
              ...
              axisLine={false}
              tickLine={false}
              tick={{ fill: "#BE185D", fontSize: 11 }}
              className="max-sm:hidden"
            />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: "rgba(244, 114, 182, 0.12)" }} />
            <Bar
              dataKey="hours"
              radius={[6, 6, 0, 0]}
              animationBegin={200}
