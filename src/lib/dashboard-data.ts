...
export function getDemoDashboardData(): DashboardData {
  ...
  const weeklyData: WeeklyDayData[] = weekDays.map((day, i) => ({
    day,
    minutes: i < todayIndex ? [90, 120, 45, 150, 75, 110, 0][i] ?? 60 : i === todayIndex ? 154 : 0,
    isToday: i === todayIndex,
    isFuture: i > todayIndex,
  }));

  const labels = [
    { id: "1", name: "Physics", color: "#7C3AED", totalMinutes: 1470, proportion: 0.42 },
    { id: "2", name: "Chemistry", color: "#06B6D4", totalMinutes: 1050, proportion: 0.3 },
    { id: "3", name: "Math", color: "#F59E0B", totalMinutes: 980, proportion: 0.28 },
  ];
  ...
  return {
    ...
    plantStage: 4,
    plantXP: 340,
    plantStageName: "Young Plant",
    nextStageXP: 500,
    lastStudiedAt: subDays(now, 0).toISOString(),
    isWilting: false,
    weeklyData,
    weekTotalHours: 14.5,
    labelBreakdown: labels.map((l) => ({
      ...
      name: l.name,
      color: l.color,
      totalMinutes: Math.round(l.proportion * 870),
    })),
    labels,
    recentSessions: [
      { id: "1", duration: 4800, mode: "pomodoro", createdAt: subDays(now, 0).toISOString(), label: { name: "Physics", color: "#7C3AED" } },
      { id: "2", duration: 3600, mode: "stopwatch", createdAt: subDays(now, 0).toISOString(), label: { name: "Chemistry", color: "#06B6D4" } },
      { id: "3", duration: 2700, mode: "pomodoro", createdAt: subDays(now, 1).toISOString(), label: { name: "Math", color: "#F59E0B" } },
      { id: "4", duration: 1500, mode: "countdown", createdAt: subDays(now, 1).toISOString(), label: null },
      { id: "5", duration: 5400, mode: "focus", createdAt: subDays(now, 2).toISOString(), label: { name: "Physics", color: "#7C3AED" } },
    ],
    ...
    heatmapData: Array.from({ length: 120 }, (_, i) => ({
      date: format(subDays(now, i), "yyyy-MM-dd"),
      minutes: [0, 15, 45, 90, 135, 180][i % 6],
    })),
    heatmapYear: now.getFullYear(),
    heatmapTotalHours: 247.5,
    heatmapActiveDays: 312,
    allTime: {
      totalHours: 247.5,
      totalSessions: 1204,
      totalPomodoros: 892,
